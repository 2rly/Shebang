...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.content !== undefined) { sets.push("content = ?"); values.push(JSON.stringify(body.content)); }
  ...
  values.push(id);
  db.prepare(`UPDATE admin_cheatsheets SET ${sets.join(", ")} WHERE id = ?`).run(...values);
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const db = getDb();
  db.prepare("DELETE FROM admin_cheatsheets WHERE id = ?").run(id);
